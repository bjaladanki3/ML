...
def basicResults(clfObj,trgX,trgY,tstX,tstY,params,clf_type=None,dataset=None):
    ...
    N = trgY.shape[0]
    print("target", np.unique(trgY))
    print("train", np.unique(tstY))
    print("training x", trgX.shape)
    print("N",N)
    temp_array = np.linspace(0.1, 1.0, 10)
    result_array = [0.03, 0.05] + temp_array.tolist()
    curve = ms.learning_curve(cv.best_estimator_,trgX,trgY,cv=5,train_sizes=result_array,verbose=10,scoring=scorer)
